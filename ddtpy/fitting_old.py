...
def regularization_penalty(galmodel, galprior, mean_gal_spec, mu_xy, mu_wave):
...
def fit_galmodel_single(galmodel, cube, atm, regparams):
    ...
    def objective_func(galparams):
        ...
        # galparams is 1-d (raveled version of galmodel)
        model.gal = galparams.reshape(galmodel.shape)

        chisq_val, chisq_grad = chisq(model, data, i_t)
        rval, rgrad = regularization_penalty(model.gal, model.galprior)
